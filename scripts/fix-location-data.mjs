...
async function fixLocationData() {
  console.log('🔧 Fixing location data for family members...\n');

  try {
    // First, let's get all family members
    ...
    // First, let's temporarily disable RLS for locations table
    console.log('🔓 Temporarily disabling RLS for locations table...');
    const { error: disableRlsError } = await supabase.rpc('exec_sql', {
      sql: 'ALTER TABLE public.locations DISABLE ROW LEVEL SECURITY;'
    });

    if (disableRlsError) {
      console.log('⚠️  Could not disable RLS (this is expected in some setups)');
    } else {
      console.log('✅ RLS disabled for locations table');
    }
    ...
    // Re-enable RLS for locations table
    console.log('\n🔒 Re-enabling RLS for locations table...');
    const { error: enableRlsError } = await supabase.rpc('exec_sql', {
      sql: 'ALTER TABLE public.locations ENABLE ROW LEVEL SECURITY;'
    });

    if (enableRlsError) {
      console.log('⚠️  Could not re-enable RLS (this is expected in some setups)');
    } else {
      console.log('✅ RLS re-enabled for locations table');
    }

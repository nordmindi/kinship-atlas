-- Production Migration: Update Database to Latest Schema
-- This migration consolidates all changes from development into a single production-ready migration
-- Created: 2025-02-01
-- 
...
--
-- Changes included:
-- 1. Role system update (admin, editor, viewer)
-- 2. Family groups system
-- 3. Story location and artifacts
-- 4. Albums system
-- 5. Admin user management functions
-- 6. User tree layouts
...
-- Create user_profiles table if it doesn't exist (for clean migrations)
...
-- ============================================================================
-- BASE TABLES (required for clean migrations)
-- ============================================================================
...
-- Ensure any NULL roles are set to 'viewer' (safety check)
UPDATE public.user_profiles 
SET role = 'viewer' 
WHERE role IS NULL;
...
-- Create indexes for story groups
...
-- Add category field to family_stories table
ALTER TABLE public.family_stories
...
-- Note: Must drop functions before recreating if return type changes
-- Using CASCADE to handle any dependencies
DO $$
BEGIN
    -- Drop get_all_users with all possible signatures
    DROP FUNCTION IF EXISTS public.get_all_users() CASCADE;
    
    -- Drop update_user_role
...
-- Fix infinite recursion: Allow users to view their own profile, admins to manage all
...
-- Use a function to check admin status to avoid infinite recursion
...
-- Update family_members RLS policies
...
-- Update family_stories RLS policies
...
-- ============================================================================
-- 9. LOCATIONS RLS FIXES FOR FAMILY GROUPS
-- ============================================================================

-- Update locations RLS policies to work with family groups
...
-- ============================================================================
-- 
-- This migration includes all schema changes from development:
-- ✓ Role system (admin, editor, viewer)
-- ✓ Family groups system
-- ✓ Story location and artifacts
-- ✓ Story groups
-- ✓ Albums system
